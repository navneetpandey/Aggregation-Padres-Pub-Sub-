...
public class SubscriptionMessage extends Message {
	...
	private long startOfTheAggregationWindow = -1; //for aggregation

	public long getStartOfTheAggregationWindow() {
		return startOfTheAggregationWindow;
	}

	public void setStartOfTheAggregationWindow(long startOfTheAggregationWindow) {
		this.startOfTheAggregationWindow = startOfTheAggregationWindow;
	}
	...
	public SubscriptionMessage duplicate() {
		SubscriptionMessage newSubscriptionMessage = new SubscriptionMessage(this.sub.duplicate(),
				this.messageID, this.lastHopID.duplicate());
		// copy general message-specific fields
		newSubscriptionMessage.nextHopID = this.nextHopID.duplicate();
		...
		newSubscriptionMessage.setStartOfTheAggregationWindow(this.startOfTheAggregationWindow);
		return newSubscriptionMessage;
	}


	public SubscriptionMessage duplicateAGG() {
		SubscriptionMessage newSubscriptionMessage = new SubscriptionMessage(this.sub.duplicate(),
				this.messageID+"x", this.lastHopID.duplicate());
		// copy general message-specific fields
		newSubscriptionMessage.nextHopID = this.nextHopID.duplicate();
		newSubscriptionMessage.ttl = this.ttl;
		newSubscriptionMessage.messageTime = (Date) this.messageTime.clone();
