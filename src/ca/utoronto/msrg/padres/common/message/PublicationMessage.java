...
public class PublicationMessage extends Message {
	...
	// WARNING: This expiry time stuff doesn't make much sense for publications
	// in our system! It's
	...
	// Aggregation
	private Set<AggregationID> onlyForTheseAggregators;
	...
	public void aggregationNotRequired() {
	 ...
	 */
	public PublicationMessage(String pubMessage)
			throws ca.utoronto.msrg.padres.common.message.parser.ParseException {
		super(pubMessage);
		publication = MessageFactory.createPublicationFromString(pubMessage
				.substring(pubMessage.lastIndexOf(":[") + 1));

		try {
			String expireTimeString = pubMessage.substring(
					pubMessage.indexOf("expiry='") + 8,
	...
	/**
	 * If you want to set the expire time with a milliseconds time value, you
	 * can
	 * 
	 * @param expireTime
	 */
	...
	public boolean isExpired() {
		if (!timedExpiry) {
			...
			if (expireNotifications == 0
					|| numNotifications < expireNotifications)
				return false;
			else
				return true;
		} else {
	...
	public PublicationMessage duplicate() {
		...
		if (this.topkSubIds != null)
			newPublicationMessage.topkSubIds = new Vector<String>(
					this.topkSubIds);

		// for aggregation
		newPublicationMessage.aggregationRequired = this.aggregationRequired;
		if (this.onlyForTheseAggregators != null)
			newPublicationMessage.onlyForTheseAggregators = new HashSet<AggregationID>(
					this.onlyForTheseAggregators);
	...
	public PublicationMessage duplicateAGG() {
		PublicationMessage newPublicationMessage = new PublicationMessage(
				this.publication.duplicate(), this.messageID+"x",
				this.lastHopID.duplicate());
		// copy general message-specific fields
		newPublicationMessage.nextHopID = this.nextHopID.duplicate();
		newPublicationMessage.ttl = this.ttl;
		newPublicationMessage.messageTime = (Date) this.messageTime.clone();
		newPublicationMessage.priority = this.priority;
		newPublicationMessage.traceRouteID = this.traceRouteID;
		...
		if (this.onlyForTheseAggregators != null) {
			newPublicationMessage.onlyForTheseAggregators = new HashSet<AggregationID>(
					this.onlyForTheseAggregators);
			newPublicationMessage.onlyForTheseAggregators.addAll(onlyForTheseAggregators);
		}
	...
	public boolean isControl() {
		...
		return val.equals("BROKER_MONITOR") || val.equals("NETWORK_DISCOVERY")
				|| val.equals("GLOBAL_FD") || val.equals("BROKER_CONTROL")
				|| val.equals("HEARTBEAT_MANAGER");
	}
	...
	public Set<AggregationID> getOnlyForTheseAggregators() {
		return onlyForTheseAggregators;
	}

	public void fwdOnlyForTheseAggregatorID(AggregationID aggregationID) {
		if (this.onlyForTheseAggregators == null)
			onlyForTheseAggregators = new HashSet<AggregationID>();
		onlyForTheseAggregators.add(aggregationID);
	}

	public void resetForwardAggregatorList() {
		this.onlyForTheseAggregators = null;
	}
