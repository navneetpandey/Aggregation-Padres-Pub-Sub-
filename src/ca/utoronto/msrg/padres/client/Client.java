...
import ca.utoronto.msrg.padres.broker.aggregation.aggregationcore.AggregationIDGenerator;
...
import ca.utoronto.msrg.padres.broker.aggregation.utility.KeyValueComparablePair;
...
public class Client {
	...
	boolean clientEdgeAggregation;

	protected ArrayList<KeyValueComparablePair> aggregatorSet;
	protected Map<Integer, Aggregator> aggregatorKeySet;
	int maxFrequency;
	...
	// String aggregation="ADAPTIVE_AGGREGATION";
	//AggregationImplementationTypes aggregationImplementationType; 
	...
	protected void initialize(ClientConfig newConfig) throws ClientException {
		...
		clientEdgeAggregation = false;
		if (clientEdgeAggregation) {
			aggregatorSet = new ArrayList<KeyValueComparablePair>();
			aggregatorKeySet = new HashMap<Integer, Aggregator>();
			maxFrequency = 1;
			subRete = new ReteNetwork(ReteNetwork.NWType.SUB_TREE, null);
			notifier = new java.util.Timer();
			agrPub = new Publication();
			notificationTask = new NotificationTask(notifier, this, agrPub);
			agID = 0;
		}
 
	/*	aggregationImplementationType = Enum.valueOf(
				AggregationImplementationTypes.class,
				clientConfig.getAggregationType());*/
	...
	protected void initLog(String logPath) throws ClientException {
		if (!Logger.getRootLogger().getAllAppenders().hasMoreElements()) {
			try {
				...
				throw new ClientException("Initialization of Logger failed: ",
						e);
			}
		}
	 ...
	 * all the clients during the initialization process.
	 * 
	 * @param cmdHandler
	 */
	 ...
	 * @return
	 */
	protected BrokerState addBrokerState(NodeAddress brokerAddress,
	...
	public void setClientID(String id) {
		...
		if (defaultBrokerAddress != null)
			defaultClientDest = MessageDestination.formatClientDestination(
					clientID, defaultBrokerAddress.getNodeURI());
	}
	...
	public BrokerState getBrokerState(String brokerURI) throws ClientException {
		try {
			...
			throw new ClientException("Could not get broker status: "
					+ e.getMessage(), e);
		}
	}
	...
	public Map<String, CompositeSubscriptionMessage> getCompositeSubscriptions()
		...
		for (BrokerState brokerState : brokerStates.values()) {
			Set<CompositeSubscriptionMessage> cSubs = brokerState
					.getCSMessages();
	...
	public void processMessage(Message msg) {
		...
		/*
		 * if(clientEdgeAggregation){ Iterator<KeyValueComparablePair> it =
		 * aggregatorSet.iterator(); Aggregator ag = null;
		 * KeyValueComparablePair kv;
		 * 
		 * Map<Publication, Set<String>> pubToSubIDMap =
		 * subRete.getMatches(receivedPubMsg.getPublication());
		 * 
		 * for (Set<String> s : pubToSubIDMap.values()) { for( String ids : s) {
		 * ag = aggregatorKeySet.get(Integer.parseInt(ids)); if(ag != null){
		 * Serializable value =
		 * receivedPubMsg.getPublication().getPairMap().get(
		 * ag.getAggregationMessage().getFieldName()); if(value != null)
		 * ag.add(msg.getMessageTime(), value.toString());
		 * 
		 * } } }
		 * 
		 * }
		...
		messageLogger.info("Message received at Client " + clientID + ": "
				+ msg);

		// TODO aggregation for adaptive and optional
		/*if ((aggregationImplementationType == AggregationImplementationTypes.ADAPTIVE_AGGREGATION || aggregationImplementationType == AggregationImplementationTypes.OPTIMAL_AGGREGATION)
				&& (receivedPubMsg.getPublication() instanceof AggregatedPublication)) {

			deliverPublicationForClient(receivedPubMsg);
	...
	public boolean checkForReceivedPub(Publication pubToCheck) {
		return brokerStates.get(defaultBrokerAddress).checkReceivedPub(
				pubToCheck);
	}
	...
	public CommandResult handleCommand(String commandString)
		...
		if (!cmdResult.isError()) {
			...
			if (handler == null) {
				cmdResult.errMsg = "Command " + cmdResult.command
	...
	public BrokerState connect(String brokerURI) throws ClientException {
		try {
			...
			if (brokerStates.containsKey(brokerAddr)) {
				...
				MessageSender msgSender = commSystem
						.getMessageSender(brokerURI);
				BrokerState bState = addBrokerState(brokerAddr, msgSender);

				msgSender.connect(MessageDestination.formatClientDestination(
						clientID, brokerAddr.getNodeURI()), msgListener);
				return bState;
			}
		} catch (CommunicationException e) {
			exceptionLogger.error("Could not connect to broker: " + e);
			throw new ClientException("Could not connect to broker: "
					+ e.getMessage(), e);
		}
	}
	...
	public String disconnectAll() {
		String outStr = "";
		for (BrokerState brokerState : new HashMap<NodeAddress, BrokerState>(
				brokerStates).values()) {
			try {
				disconnect(brokerState);
	 ...
	 *             communication error.
	 */
	protected BrokerState disconnect(BrokerState brokerState)
			throws ClientException {
		try {
			...
			msgSender.disconnect(MessageDestination.formatClientDestination(
	...
	public AdvertisementMessage advertise(String advStr, String brokerURI)
		...
		Advertisement newAdv = MessageFactory
				.createAdvertisementFromString(advStr);
	 ...
	 * @see #advertise(Advertisement, String)
	 */
	public AdvertisementMessage advertise(Advertisement adv)
			throws ClientException {
	...
	public AdvertisementMessage advertise(Advertisement adv, String brokerURI)
		...
		try {
			...
			BrokerState brokerState = getBrokerState(brokerURI);
			if (brokerState == null) {
				throw new ClientException("Not connected to broker "
						+ brokerURI);
			}

			MessageDestination clientDest = MessageDestination
					.formatClientDestination(clientID, brokerState
							.getBrokerAddress().getNodeURI());
			AdvertisementMessage advMsg = new AdvertisementMessage(adv,
					getNextMessageID(brokerState.getBrokerAddress()
							.getNodeURI()), clientDest);
			String msgID = brokerState.getMsgSender().send(advMsg,
					HostType.CLIENT);
	...
	public SubscriptionMessage subscribe(String subStr, String brokerURI)
			throws ClientException, ParseException {
		Subscription newSub = MessageFactory
		...
		// For aggregation
		Predicate aggOp = newSub.getPredicateMap().remove("AGR");
		if (aggOp != null) {
			Predicate aggPAR = newSub.getPredicateMap().remove("PAR");
			Predicate aggPRD = newSub.getPredicateMap().remove("PRD");
			Predicate aggShift = newSub.getPredicateMap().remove("NTF");
			/*
			 * if(clientEdgeAggregation) { //AggregationMessage String
			 * whereToNotify = (agID++).toString(); //msg.getMessageID(); int
			 * period = Integer.parseInt(aggPRD.getValue().toString()); int ntf
			 * = Integer.parseInt(aggNTF.getValue().toString()); Aggregator
			 * agtr=
			 * AggregatorFactory.createAggregator(AggregationOperator.fromString
			 * (aggOp.getValue().toString()),AggregationType.TIMEBASED,
			 * whereToNotify, newSub.getPredicateMap() ,
			 * aggPAR.getValue().toString(), period,
			 * Integer.parseInt(aggNTF.getValue().toString()), false,"");
			 * aggregatorSet.add(new KeyValueComparablePair(ntf,agtr));
			 * aggregatorKeySet.put(agID,agtr); Collections.sort(aggregatorSet);
			 * if(period > maxFrequency) maxFrequency = period; try {
			 * subRete.addSub(newSub, agID.toString()); } catch (ReteException
			 * e) {
			 * 
			 * e.printStackTrace(); } } else {
			 */
			long windowSize = Long.parseLong(aggPRD.getValue().toString());
			long shiftSize = Long.parseLong(aggShift.getValue().toString());
			if (clientConfig.aggregationON) {
				newSub.setAggregationID(new AggregationID(newSub
						.getPredicateMap(), windowSize, shiftSize, aggPAR
						.getValue().toString(), OperatorType.fromString(aggOp
						.getValue().toString()),
				// newSub.getSubscriptionID()
						"" + System.nanoTime()));
				newSub.setAggregation(true);
			}
			// }
		}
	 ...
	 *             an exception.
	 */
	public SubscriptionMessage subscribe(Subscription sub)
			throws ClientException {
	 ...
	 *             </ul>
	 */
	public SubscriptionMessage subscribe(Subscription sub, String brokerURI)
			throws ClientException {
		if (!isConnected())
			throw new ClientException("Not connected to any broker");
		try {
			...
			String msgID = brokerState.getMsgSender().send(subMsg,
					HostType.CLIENT);
			...
			/*if (subMsg.getSubscription().isAggregation()
					&& ((aggregationImplementationType == AggregationImplementationTypes.ADAPTIVE_AGGREGATION || aggregationImplementationType == AggregationImplementationTypes.OPTIMAL_AGGREGATION))) {
				addClient(subMsg);
			}*/
	...
	public CompositeSubscriptionMessage subscribeCS(String subStr,
			String brokerID) throws ClientException {
		// create the subscription
		CompositeSubscription newCS = new CompositeSubscription(subStr);
		if (newCS.getSubscriptionMap() == null
				|| newCS.getSubscriptionMap().size() == 0) {
	...
	public CompositeSubscriptionMessage subscribeCS(CompositeSubscription cs,
			String brokerURI) throws ClientException {
		if (!isConnected())
			throw new ClientException("Not connected to any broker");
		try {
			...
			MessageDestination clientDest = MessageDestination
					.formatClientDestination(clientID, brokerState
							.getBrokerAddress().getNodeURI());
			CompositeSubscriptionMessage newCSMsg = new CompositeSubscriptionMessage(
					cs, getNextMessageID(brokerState.getBrokerAddress()
							.getNodeURI()), clientDest);
			String msgID = brokerState.getMsgSender().send(newCSMsg,
					HostType.CLIENT);
	...
	public PublicationMessage publish(String pubStr, String brokerID)
			throws ClientException {
		try {
			Publication newPub = MessageFactory
					.createPublicationFromString(pubStr);
	...
	public PublicationMessage publish(String pubStr, Serializable payload,
			String brokerID) throws ClientException {
		try {
			Publication newPub = MessageFactory
					.createPublicationFromString(pubStr);
	...
	public PublicationMessage publish(String pubStr,
			ConcurrentHashMap<Object, Object> payload, String brokerID)
			throws ClientException {
	...
	public PublicationMessage publish(Publication pub, String brokerURI)
			throws ClientException {
		if (!isConnected())
			throw new ClientException("Not connected to any broker");
		try {
			...
			PublicationMessage pubMsg = new PublicationMessage(pub,
					getNextMessageID(brokerState.getBrokerAddress()
							.getNodeURI()), clientDest);
			pubMsg.setAggregationRequired(true);
			pubCount++;
			String msgID = brokerState.getMsgSender().send(pubMsg,
					HostType.CLIENT);
	...
	protected void unAdvertiseAll(BrokerState brokerState)
			throws ClientException {
		if (!clientConfig.detailState)
			throw new ClientException(
		...
		AdvertisementMessage[] advMsgArray = brokerState.getAdvMessages()
				.toArray(new AdvertisementMessage[0]);
	...
	public UnadvertisementMessage unAdvertise(String advID)
			throws ClientException {
	...
	public List<UnadvertisementMessage> unAdvertise(String[] advIDList)
			throws ClientException {
	...
	protected UnadvertisementMessage unAdvertise(String advID,
			BrokerState brokerState) throws ClientException {
		try {
			...
			UnadvertisementMessage unAdvMsg = new UnadvertisementMessage(unAdv,
					getNextMessageID(brokerState.getBrokerAddress()
							.getNodeURI()), clientDest);
			String msgID = brokerState.getMsgSender().send(unAdvMsg,
					HostType.CLIENT);
	...
	protected void unsubscribeAll(BrokerState brokerState)
			throws ClientException {
		if (!clientConfig.detailState)
			throw new ClientException(
		...
		if (msgSender == null)
			throw new ClientException("Connection not found for broker "
					+ brokerState.getBrokerAddress());
		SubscriptionMessage[] subMsgArray = brokerState.getSubMessages()
				.toArray(new SubscriptionMessage[0]);
	...
	public UnsubscriptionMessage unSubscribe(String subID)
			throws ClientException {
	...
	public List<UnsubscriptionMessage> unSubscribe(String[] subIDList)
			throws ClientException {
	...
	protected UnsubscriptionMessage unSubscribe(String subID,
			BrokerState brokerState) throws ClientException {
		try {
			...
			UnsubscriptionMessage unSubMsg = new UnsubscriptionMessage(unSub,
					getNextMessageID(brokerState.getBrokerAddress()
							.getNodeURI()), clientDest);
			String msgID = brokerState.getMsgSender().send(unSubMsg,
					HostType.CLIENT);
	...
	protected void unsubscribeCSAll(BrokerState brokerState)
			throws ClientException {
		if (!clientConfig.detailState)
			throw new ClientException(
		...
		for (CompositeSubscriptionMessage csMsg : csMsgArray) {
			Uncompositesubscription unCS = new Uncompositesubscription(
					csMsg.getMessageID());
			UncompositesubscriptionMessage unCSMsg = new UncompositesubscriptionMessage(
					unCS, getNextMessageID(brokerState.getBrokerAddress()
							.getNodeURI()), clientDest);
			try {
	...
	public UncompositesubscriptionMessage unSubscribeCS(String csID)
			throws ClientException {
	...
	protected UncompositesubscriptionMessage unSubscribeCS(String csID,
			BrokerState brokerState) throws ClientException {
		try {
			...
			UncompositesubscriptionMessage unCSMsg = new UncompositesubscriptionMessage(
					unCS, getNextMessageID(brokerState.getBrokerAddress()
							.getNodeURI()), clientDest);
			String msgID = brokerState.getMsgSender().send(unCSMsg,
					HostType.CLIENT);
	 ...
	 * unique because of the message serial number. The ID has the format of
	...
	/*
	 * public void notification( int count, Publication agrPub) {
	 * 
	 * Iterator<Integer> iter = aggregatorKeySet.keySet().iterator(); Integer
	 * key; while(iter.hasNext()) { key = iter.next(); Aggregator aggregator =
	 * (Aggregator)aggregatorKeySet.get(key); if
	 * ((aggregator.getAggregationMessage().getNotificaitonFrequency() == count)
	 * && !aggregator.hasNoPublication()){ AggregateComputer am =
	 * aggregator.getAggregationMessage();
	 * 
	 * try { synchronized(notificationTask) { Publication newPub =
	 * MessageFactory
	 * .createPublicationFromString("[class,'STOCK'],[symbol,'IBM']");
	 * newPub.getPairMap
	 * ().put(am.getOperator().convertString(),aggregator.publishResult(true));
	 * newPub.getPairMap().put("AGR_ON", am.getFieldName());
	 * agrPub.setPairMap(newPub.getPairMap()); notificationTask.notify(); } }
	...
	public void notificationThread(Publication printPub,
			AggregationPrinter aggregationPrinter) {

		int tickFrequencey = 10000;// in millisecond
		notifier.schedule(notificationTask, 0, tickFrequencey);

		Thread t = new Thread(new NotificationThread(printPub, agrPub,
				aggregationPrinter, notificationTask), "notifier");
		t.start();
