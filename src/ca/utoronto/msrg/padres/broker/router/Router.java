...
public abstract class Router {
	...
	public Set<Message> handleMessage(Message msg) {
		...
		try {
			...
			if (type.equals(MessageType.ADVERTISEMENT)) {
				...
				Stats.getInstance("test").addValue("Matching.duration", System.currentTimeMillis() - startTime);
	...
	protected Set<Message> handleMessage(AdvertisementMessage advMsg)
		...
		System.out.println(" ADVERTSIMENT " + advMsg.getAdvertisement().toString());
		return messagesToRoute;
	}
	...
	protected Set<Message> handleMessage(PublicationMessage pubMsg)
		...
		//Navneet aggregation check
		//Extract out the aggregation key value pair
	//	PublicationMessage aggrePubMsg = pubMsg;
		//Publication withAggrePub = pubMsg.getPublication();
	//	Publication withOutAggrePub;
		//Map<String, Serializable> pairMap = withAggrePub.getPairMap();
		//Serializable aggVal = pubMsg.getPublication().getPairMap().remove("distinct");

	/*	if(aggVal != null)
	{
			//withOutAggrePub = new Publication();
		//	withOutAggrePub.setPairMap(pairMap);
			pubMsg.getPublication().setPairMap(pairMap);
		}*/
//		else 
//		{
//			withOutAggrePub = withAggrePub;
//		}
		...
		//navneet setting it back
		//if(aggVal != null)
			//pubMsg.getPublication().getPairMap().put("distinct", aggVal);
		...
		//aggregation
		//postprocessor.getAggregationEngine().processPerMessage(pubMsg, pubMsgToSubIDs);
