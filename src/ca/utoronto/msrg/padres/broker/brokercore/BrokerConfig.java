...
/**
 * The data structure to process broker configuration files and command line
 * options and store the broker configurations.
 * 
...
public class BrokerConfig {
	...
	protected static final String DEFAULT_MI_PROPS_FILE_PATH = PADRES_HOME
			+ "etc" + File.separator + "mi.properties";
	...
	private static final String CMD_ARG_FLAG_AGGREGATION = "aggregation";

	private static final String CMD_ARG_FLAG_ADAPTATIONFORAGGREGATION = "adaptationForAggregation";
	...
	private AggregationInfo aggregationInfo = null;
	private String statsDirectory ="";
	
	private boolean adaptationForAggregation = false;
	
	private int experimentDuration = 1000;
	...
	public BrokerConfig() throws BrokerCoreException {
		...
		try {
			...
			throw new BrokerCoreException(
					"Cannot load broker properties file: ", e);
		}
	}

	public BrokerConfig(String configPath, boolean def)
	...
	public BrokerConfig(BrokerConfig origConfig) {
		...
		this.neighborURIs = Arrays.copyOf(origConfig.neighborURIs,
				origConfig.neighborURIs.length);
		this.connectionRetryLimit = origConfig.connectionRetryLimit;
		this.connectionRetryPause = origConfig.connectionRetryPause;
		this.managers = Arrays.copyOf(origConfig.managers,
				origConfig.managers.length);
		...
		this.aggregation = origConfig.aggregation;
		this.aggregationInfo = origConfig.aggregationInfo;
		this.statsDirectory = origConfig.statsDirectory;
		this.adaptationForAggregation = origConfig.adaptationForAggregation;
		this.experimentDuration = origConfig.experimentDuration;
	}
	...
	protected void convertProperties(Properties properties) {
		...
		matcherName = MatcherType.valueOf(properties.getProperty(
		...
		pubConformCheck = properties.getProperty("padres.pub.conformcheck")
				.equals("ON") ? true : false;

		cycleOption = CycleType.valueOf(properties.getProperty("padres.cycles",
				"OFF"));
		...
		heartBeat = properties.getProperty("padres.heartbeat").equals("ON") ? true
				: false;

		msgTrace = properties.getProperty("padres.traceall").equals("ON") ? true
				: false;

		cliInterface = properties.getProperty("padres.consoleinterface")
				.equals("ON") ? true : false;

		webInterface = properties.getProperty(
				ManagementServer.PROP_WEB_MANAGEMENT).equals("ON") ? true
				: false;

		webPort = Integer.parseInt(properties.getProperty(
				ManagementServer.PROP_WEB_PORT,
		...
		managementInterface = properties.getProperty(
				"padres.managementinterface").equals("ON") ? true : false;

		managementPropertyFileName = properties
				.getProperty("padres.mipropertyfile");
		if (managementPropertyFileName == null)
			managementPropertyFileName = DEFAULT_MI_PROPS_FILE_PATH;

		monitorBrokerInfo = properties.getProperty(
				BrokerInfoPublisher.PROP_BROKER_INFO,
				BrokerInfoPublisher.BROKER_INFO_OFF).equals(
				BrokerInfoPublisher.BROKER_INFO_ON) ? true : false;

		monitorBrokerInfoAdvSubInfoType = AdvSubInfoType.valueOf(properties
				.getProperty(SystemMonitor.PROP_ADV_SUB_INFO_TYPE,
						AdvSubInfoType.FULL.toString()));

		totalOrder = properties.getProperty("padres.order").equals("ON") ? true
				: false;

		topk = properties.getProperty("padres.topk").equals("ON") ? true
				: false;

		topkInfo = new TopkInfo(Integer.parseInt(properties.getProperty(
				"padres.k", "0")), Integer.parseInt(properties.getProperty(
				"padres.W", "0")), Integer.parseInt(properties.getProperty(
				"padres.shift", "0")), Integer.parseInt(properties.getProperty(
				"padres.chunk", "0")));

		aggregation = properties.getProperty("padres.aggregation").equals("ON") ? true
				: false;
		
		experimentDuration = Integer.parseInt(properties.getProperty(
				"aggregation.experimentDuration", "1000"));
		
		String implementation = System
				.getProperty("padres.aggregation.implementation");
		if (implementation == null)
			implementation = properties.getProperty("padres.aggregation.implementation");
		
		String clientAggregationString = System
				.getProperty("aggregation.client");
		
		if(clientAggregationString == null)
			clientAggregationString = properties.getProperty("aggregation.client");
		
		boolean clientAggregation = clientAggregationString.equals("ON")?true:false;
		
		aggregationInfo = new AggregationInfo(
				Integer.parseInt(properties.getProperty("padres.aggregation.timeout", "100")),
				Enum.valueOf(AggregationImplementationTypes.class, implementation), clientAggregation);
		
		statsDirectory = properties.getProperty(
				"padres.stats.basedir","");
		
		adaptationForAggregation = properties.getProperty(
				"padres.adaptationForAggregation").equals("ON") ? true : false;
	}
	...
	public static String[] getCommandLineKeys() {
		...
		cliKeys.add(CMD_ARG_FLAG_AGGREGATION + ":");
		cliKeys.add(CMD_ARG_FLAG_ADAPTATIONFORAGGREGATION + ":");
	...
	protected void overwriteWithCmdLineArgs(CommandLine cmdLine) {
		...
		if ((buffer = cmdLine.getOptionValue(CMD_ARG_FLAG_MON_BI)) != null)
			monitorBrokerInfo = buffer.trim().equals(
					BrokerInfoPublisher.BROKER_INFO_ON) ? true : false;
		if ((buffer = cmdLine.getOptionValue(CMD_ARG_FLAG_MON_BI_ASINFO)) != null)
			monitorBrokerInfoAdvSubInfoType = AdvSubInfoType.valueOf(buffer
					.trim());
		...
		if ((buffer = cmdLine.getOptionValue(CMD_ARG_FLAG_AGGREGATION)) != null)
			aggregation = buffer.trim().equals("ON") ? true : false;
		if ((buffer = cmdLine
				.getOptionValue(CMD_ARG_FLAG_ADAPTATIONFORAGGREGATION)) != null)
			adaptationForAggregation = buffer.trim().equals("ON") ? true
					: false;
	}
	...
	public boolean checkConfig() throws BrokerCoreException {
		if (brokerURI == null || brokerURI.length() == 0) {
			throw new BrokerCoreException(
					"Missing uri key or uri value in the property file.");
		}
		return true;
	}
	...
	public boolean isCycle() {
		return cycleOption == CycleType.DYNAMIC
				|| cycleOption == CycleType.FIXED;
	}
	...
	public String toString() {
		...
		outString += "\nAdaptationForAggregation: " + adaptationForAggregation;
		return outString + "\n";
	}
	...
	public boolean isAdaptationForAggregation() {
		return adaptationForAggregation;
	}
